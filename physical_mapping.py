...
def drawShapes(shapes):
    for p in range(0,434):
        ...
        b = 0
        for shape in shapes:
            if (shape.contains(pmap[p][0], pmap[p][1])):
...
def drawShapeBorders(shapes, borderWidth, backgroundColor):
    for p in range(0,434):
        ...
        b = 0
        for shape in shapes:
            if (shape.containsInBorder(pmap[p][0], pmap[p][1], borderWidth)):
                numColors = numColors + 1
                r = r + (shape.color[0] * shape.color[0])
                g = g + (shape.color[1] * shape.color[1])
                b = b + (shape.color[2] * shape.color[2])
...
def drawRainingSquares():
    ...
    fallSpeed = []
    for i in range(0,10):
        squares.append(Square(i*10,random.randint(0,SCREEN), wheel(random.randint(0, 253)), 20))
        fallSpeed.append(random.randint(1,10))
    while run:
        drawShapes(squares)
        ...
        for j in range(0, len(squares)):
            ...
            if squares[j].isOffscreen(SCREEN, SCREEN):
                squares[j].y = 0
                fallSpeed[j] = random.randint(1,10)
...
def expandingSquares():
    ...
    while run:
        ...
        for j in range(0, len(squares)):
            ...
            if expandLength[j] < 0:
                squares[j] = Circle(random.randint(0,SCREEN),random.randint(0,SCREEN), colors[random.randint(0, 8)], 1)
